...
export const JobPreviewCard = ( {formDeets} ) => {
  ...
  const { title, jobLocation, salary, jobType, sector, companyName, salaryStructures} = formDeets
  ...
  return (
    <Box position="relative" rounded="md" bg="gray.50" w="100%">
      <VStack p="3" spacing="1" align="flex-start">
        ...
        <Text as="h3" fontSize="lg" textTransform="uppercase" color="gray.600" letterSpacing="normal">
          {sector}
        </Text>
        <HStack spacing="1">
          <Icon as={HiCash} fontSize="xl" color="gray.400" />
          <Text
            ...
            color={useColorModeValue('gray.600', 'gray.300')}
          >
            {salary && (`£${formattedSalary}`)} / {salaryStructures?.toLocaleLowerCase()}
          </Text>
        </HStack>
        <HStack spacing="1">
          <Icon as={HiClock} color="gray.400" />
